...
self.addEventListener('activate', event => {
	...
	event.waitUntil(caches.match('/apps.json')
			.then(response => response.json())
			.then(config => portalConfig = config));
...
self.addEventListener('fetch', function (event) {
	...
	if (event.request.method === "GET") {
		console.log("SW-GET:" + event.request.url, portalConfig);
		let url = new URL(event.request.url);
		if (url.origin === location.origin) {
			const targetApps = portalConfig.filter(app => url.pathname.startsWith(app.target_path));//TODO warning with prefix-paths like /app and /app1
			if (targetApps.length > 0) {
				...
				console.log(`found non-navigation frontend request! Rewrite fetch to ${url}`);
